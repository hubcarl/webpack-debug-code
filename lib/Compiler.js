...
class Compiler extends Tapable {
	...
	run(callback) {
		...
		this.applyPluginsAsync("before-run", this, err => {
			//console.log('---applyPluginsAsync:before-run');
			if(err) return callback(err);
			...
			this.applyPluginsAsync("run", this, err => {
				...
				this.readRecords(err => {
					//console.log('---applyPluginsAsync:readRecords');
					if(err) return callback(err);
	...
	plugin(name, fn){
		...
		try{
			...
			const result = e.stack.match(/at ([A-Za-z]+)\.apply/);
			if(Array.isArray(result) && result.length >1){
				console.log((++registerOrder) + ':' + 'event-register-plugin-mapping:'+ name + ':' + result[1], );
				fn.__pluginName__ = result[1];
			} else {
				//console.log('----name:', name,  e.stack);
			}

			super.plugin(name, fn);
		}
	}
