...
function copyProperties(from, to) {
	if(flag ===1){
		print('copyProperties from', from);
		print('copyProperties to', to);
	}
...
function printEventPluginName(fn, name){
	if(fn && fn.__pluginName__){
		console.log((++order) + ':' + 'event-plugin-mapping:'+ fn.__pluginName__ + ':' + name, );
	}
}
...
Tapable.prototype.applyPlugins = function applyPlugins(name) {
	...
	for(var i = 0; i < plugins.length; i++){
		printEventPluginName(plugins[i], name);
...
Tapable.prototype.applyPluginsAsyncSeries = Tapable.prototype.applyPluginsAsync = function applyPluginsAsyncSeries(name) {
	...
	printEventPluginName(plugins[0], name);
...
Tapable.prototype.apply = function apply() {
	for(var i = 0; i < arguments.length; i++) {
		//console.log(`Tapable.apply#call plugin apply method#plugin:${arguments[i].constructor.name},source:${arguments[i].source},target:${arguments[i].target}`,);
		//printEventPluginName(arguments[i], name);
